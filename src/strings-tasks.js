...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === str[i].toUpperCase()) {
      invertedStr.push(str[i].toLowerCase());
    } else {
      invertedStr.push(str[i].toUpperCase());
    }
  }
...
function extractNameFromTemplate(value) {
  ...
  const surname = value.split(' ')[2].slice(0, -1);
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (alphabet.includes(str[i])) {
      encodedStr.push(rot13[alphabet.indexOf(str[i])]);
    } else {
...
function getCardId(value) {
  ...
  const cardValueIndex = cards.indexOf(cardValue);
  const cardSuitIndex = suits.indexOf(cardSuit);

  return cardValueIndex + cardSuitIndex * cards.length;
}
